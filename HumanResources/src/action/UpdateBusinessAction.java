 ...
 * 
 */
public class UpdateBusinessAction extends ActionSupport {
	...
	public String execute() throws Exception {
		...
		Employees_InformationBean eiBean1 = eDao.getUserBeanById(id);
		if ("0".equals(eiBean1.getEmployees_status())) {
			actionContext.put("Error", "该员工已被删除,不能修改！");
			return "error";
		}
		...
		if (d1.getTime() > d2.getTime()) {
			...
			if (this.getUpload() != null) {
				...
				eiBean.setEmployees_photo(this.getTupian());
			}
			// 职务表
			...
			boolean flag = eDao.updateoper_PRIVById(eiBean);
			if (flag) {
				actionContext.put("SuccessOpt", "updata");
				return "success";
			} else {
				actionContext.put("Error", "修改操作员失败！");
				return "error";
			}
		}
